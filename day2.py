...
def convert_input(input):
    def map_line(line):
        result = re.match(r'(?P<int_1>\d+)-(?P<int_2>\d+) (?P<char>[a-z]): (?P<password>[a-z]+)', line)
        return (int(result.group('int_1')), int(result.group('int_2'))), result.group('char'), result.group('password')

    return map(map_line, input.strip().split('\n'))


def part1(input):
    converted_input = convert_input(input)
    valid_password = 0
    for policy_range, policy_char, password in converted_input:
        if policy_range[0] <= password.count(policy_char) <= policy_range[1]:
            valid_password += 1
...
def part2(input):
    converted_input = convert_input(input)
    valid_password = 0
    for policy_positions, policy_char, password in converted_input:
        policy_positions = set(policy_positions)
        policy_char_occurrences = [((password[index] == policy_char) and (index + 1 in policy_positions))
                                   for index in range(len(password))].count(True)
        if policy_char_occurrences == 1:
            valid_password += 1
    return valid_password
