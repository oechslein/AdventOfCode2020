...
class Tile(object):
    ...
    def as_tuple(self):
        return tuple(''.join(str(tile_line)) for tile_line in self.tile_lines)
    ...
    def get_border(self, border: int):
        if border == 0:
            ...
            return ''.join(np.rot90(self.tile_lines, k=-1, axes=(1, 0))[0])
        elif border == 2:
            return ''.join(self.tile_lines[self.width - 1])
        elif border == 3:
            return ''.join(np.rot90(self.tile_lines, k=-1, axes=(1, 0))[self.height - 1])
