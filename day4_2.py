...
password_field_checker = {
    'byr': lambda data: re.fullmatch(r'(\d{4})', data) and 1920 <= int(data) <= 2002,
    'iyr': lambda data: re.fullmatch(r'(\d{4})', data) and 2010 <= int(data) <= 2020,
    'eyr': lambda data: re.fullmatch(r'(\d{4})', data) and 2020 <= int(data) <= 2030,
    'hgt': lambda data: ((result := re.fullmatch(r'(\d{2,3})(cm|in)', data))
                         and ((result.group(2) == 'cm' and 150 <= int(result.group(1)) <= 193)
                              or (result.group(2) == 'in' and 59 <= int(result.group(1)) <= 76))),
    'hcl': lambda data: re.fullmatch(r'#[0-9a-f]{6}', data),
    'ecl': lambda data: data in {'amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'},
    'pid': lambda data: re.fullmatch(r'\d{9}', data),
    'cid': lambda data: True,
}


def is_valid_password_dict(password_dict, needed_keys):
    def _has_needed_keys():
        return set(password_dict.keys()).issuperset(needed_keys)

    def _field_data_correct():
        return all(password_field_checker[field](data) for field, data in password_dict.items())

    return _has_needed_keys() and (_field_data_correct())


def gen_valid_passwords(my_input, needed_keys):
    return (password_dict
            for password_dict in convert_input(my_input)
            if is_valid_password_dict(password_dict, needed_keys))


def count_valid_passwords(my_input, needed_keys):
    return sum(1 for _ in gen_valid_passwords(my_input, needed_keys))


needed_keys = {'hcl', 'byr', 'ecl', 'cid', 'hgt', 'iyr', 'eyr', 'pid'} - {'cid'}
assert count_valid_passwords(test_input, needed_keys=needed_keys) == 2

assert password_field_checker['byr']('2002')
assert not password_field_checker['byr']('2003')
assert password_field_checker['hgt']('60in')
assert password_field_checker['hgt']('190cm')
assert not password_field_checker['hgt']('190in')
assert not password_field_checker['hgt']('190')
assert password_field_checker['ecl']('brn')
assert not password_field_checker['ecl']('wat')
assert password_field_checker['pid']('000000001')
assert not password_field_checker['pid']('0123456789')

print(count_valid_passwords(puzzle_input, needed_keys=needed_keys))
